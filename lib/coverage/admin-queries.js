...
export async function getClassOccurrenceDates(classId, startDate, targetSessions, days) {
  ...
  const result = computeClassOccurrences({
    ...
    days,
    targetSessions,
    holidays: (holidays || []).map((h) => ({ date: h.date, name: "Holiday" })),
    programOffDays: (offDays || []).map((o) => ({ date: o.date, reason: "Program Off Day" })),
  });

  // Filter to future dates and exclude already covered dates
  const futureDates = result.occurrences
    .filter((o) => o.date >= today && !coveredDates.has(o.date))
    .map((o) => o.date)
