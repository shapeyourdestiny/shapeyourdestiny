...
export async function getMyCoverageRequests(profileId) {
  ...
  // Get requests I made (that are still upcoming)
  ...
  if (myError) {
    console.error("Error fetching my coverage requests:", myError);
  }
...
/**
 * Check if a specific session has an open or claimed coverage request.
 * Used to determine if "Can't make it?" button should be shown.
 */
export async function getCoverageStatusForSession(classId, date, profileId) {
  const supabase = await createClient();

  const { data, error } = await supabase
    .from("coverage_requests")
    .select("id, status, requested_by, claimed_by")
    .eq("class_id", classId)
    .eq("date", date)
  ...
  if (!data) {
    return { hasCoverageRequest: false };
  }

  return {
    hasCoverageRequest: true,
    status: data.status,
    isMyRequest: data.requested_by === profileId,
    isCoveredByMe: data.claimed_by === profileId,
    coverageId: data.id,
  };
}
